...
def view_trend_at_times(record_dict):
    ...
    for direction in arg.keys():
        ...
        '''
        oc.figure()
        oc.plot(*(arg[direction][:8*3]))
        oc.legend(*(legend[direction][:8]))
        oc.title(direction)
        
        oc.figure()
        oc.plot(*(arg[direction][8*3:8*6]))
        oc.legend(*(legend[direction][8:8*2]))
        oc.title(direction)
 
        oc.figure()
        oc.plot(*(arg[direction][8*6:]))
        oc.legend(*(legend[direction][8*2:]))
        oc.title(direction)
